...
# New models which were missing as a  Foreign Key reference
...
class CRMInv(models.Model):
    ...
    tax_percent = models.DecimalField(max_digits=5, decimal_places=2, db_column='fTaxPerc', default=0.05) #default should be 0.05 
    """
    In the initial model, there was an issue with the tax_percent field definition. It was set max_digits to an empty value, which is not valid for a DecimalField. To rectify this, I needed to specify both max_digits and decimal_places. In this case, I wanted to allow up to 5 digits with 2 decimal places for the tax percentage, so I defined it like this:
    """
...
class CRMPil(models.Model):  
    ...
    probability_percent = models.IntegerField(db_column='nProbPerc', default=0)  # Fixed the default value
    """
    In the corrected code, I've provided a default value of 0 for the probability_percent field, which ensures that it has a valid default value.
    """
